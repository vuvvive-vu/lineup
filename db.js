...
async function getUserById(id) {
  const { rows } = await pool.query(
    'SELECT id, username, display_name, email, avatar, bio, email_verified, badge, badges, active_badge, created_at FROM users WHERE id=$1',
    [id]
  );
  return rows[0] || null;
}
...
async function getUserByUsername(username) {
  const { rows } = await pool.query(
    'SELECT id, username, display_name, email, avatar, bio, email_verified, badge, badges, active_badge, created_at FROM users WHERE lower(username)=lower($1) ORDER BY created_at DESC LIMIT 1',
    [username]
  );
  return rows[0] || null;
}
...
async function getUserByEmail(email) {
  const { rows } = await pool.query(
    'SELECT id, username, display_name, email, password_hash, avatar, bio, email_verified, badge, badges, active_badge, verify_token, reset_token, reset_expires, created_at FROM users WHERE email=$1 LIMIT 1',
    [email]
  );
  return rows[0] || null;
}
